...
class GoToNode(PipelineRouter):
    ...
    def get_next(cls, output, context: dict, pipeline_obj):
        ...
        if 'goto_node_id' in context.keys():
            _next_id = str(context.pop('goto_node_id'))
        elif 'goto_node_name' in context.keys():
            _node_name = context.pop('goto_node_name')
            _next_id = Tools.get_node_id_by_name(_node_name, pipeline_obj)
            if _next_id is None:
                raise RuntimeError(
                    'GoToNode Router Error: goto_node_name[%s] not found!' % _node_name)
        else:
            raise RuntimeError('GoToNode Router Error: goto para not found in context!')
